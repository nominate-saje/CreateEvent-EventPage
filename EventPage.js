...
const sampleEventId = "MYUYf0g9xJbYiXmc92M5"; // todo: delete this line
...
export default class EventPage extends Component {
  ...
  getEventData(cb = () => {}) {
    fetch(`https://us-central1-nominate-hr.cloudfunctions.net/api/events/${sampleEventId}`, { // TODO sampleEventId => this.props.navigation.geparam?? ?? ? ?
      method: "GET"
    }).then(res => res.json())
    .then(response => {
          ...
          <ScrollView style={styles.events} bounces="false">
            ...
            <View style={styles.ihatethis}>
              {this.state.restaurants.map((nom, i) => (
                <Nomination nom={nom} eventId={sampleEventId} update={this.getEventData} index={i} key={i}/>
              ))} 
            </View>
